...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		if stickyState.pinnedAuthID != "" {
			pickOpts = withPinnedAuthMetadata(pickOpts, stickyState.pinnedAuthID)
		}
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		streamResult, errStream := m.executeStreamWithModelPool(execCtx, executor, auth, provider, execReq, pickOpts, routeModel, models, pooled, prewarmSelection)
		if errStream != nil {
			if errCtx := execCtx.Err(); errCtx != nil {
				return nil, errCtx
			}
...
func (m *Manager) tryAntigravityCreditsExecuteStream(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, bool) {
	...
	for _, c := range candidates {
		...
		result, errStream := m.executeStreamWithModelPool(creditsCtx, c.executor, c.auth, c.provider, req, creditsOpts, routeModel, models, len(models) > 1, false)
		if errStream != nil {
			continue
		}
		return result, true
