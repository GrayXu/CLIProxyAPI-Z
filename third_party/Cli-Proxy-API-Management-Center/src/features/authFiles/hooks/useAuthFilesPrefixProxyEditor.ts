...
const buildAuthFileFieldsPatch = (
  ...
  const nextDisableCooling =
    disableCoolingText === '' ? undefined : parseDisableCoolingValue(disableCoolingText);
  if (disableCoolingText === '') {
    if (originalDisableCooling !== undefined) {
      patch.disable_cooling = null;
    }
  } else if (nextDisableCooling !== undefined && nextDisableCooling !== originalDisableCooling) {
  ...
  const openPrefixProxyEditor = async (file: AuthFileItem) => {
    ...
    if (prefixProxyEditor?.fileName === name) {
      ...
      setPrefixProxyEditor({
        ...
        isCodexFile: fileProviderKey === 'codex',
        loading: true,
      saving: false,
      error: null,
      originalText: '',
