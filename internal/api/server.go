...
func hasViewerManagementKey(cfg *config.Config) bool {
	if cfg == nil {
		return false
	}
...
func (s *Server) registerManagementRoutes() {
	...
	protected.GET("/xai-auth-url", s.mgmt.RequestXAIToken)
	...
	admin.Use(s.mgmt.RequireAdmin())
	{
		...
		protected.GET("/plugins", s.mgmt.ListPlugins)
		protected.GET("/plugin-store", s.mgmt.ListPluginStore)
		protected.GET("/plugins/:id/config", s.mgmt.GetPluginConfig)
		...
		admin.POST("/plugin-store/:id/install", s.mgmt.InstallPluginFromStore)
		admin.DELETE("/plugins/:id", s.mgmt.DeletePlugin)
		admin.PATCH("/plugins/:id/enabled", s.mgmt.PatchPluginEnabled)
		admin.PUT("/plugins/:id/config", s.mgmt.PutPluginConfig)
		admin.PATCH("/plugins/:id/config", s.mgmt.PatchPluginConfig)
		...
		admin.GET("/api-key-usage", s.mgmt.GetAPIKeyUsage)
		admin.GET("/usage-queue", s.mgmt.GetUsageQueue)
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if s.envManagementSecret {
		...
		case !prevRoutesEnabled && newRoutesEnabled:
			s.registerManagementRoutes()
			if s.managementRoutesEnabled.CompareAndSwap(false, true) {
				log.Info("management routes enabled after access key update")
			} else {
				s.managementRoutesEnabled.Store(true)
			}
		case prevRoutesEnabled && !newRoutesEnabled:
			if s.managementRoutesEnabled.CompareAndSwap(true, false) {
				log.Info("management routes disabled after access key removal")
			} else {
